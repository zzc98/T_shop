	...
	<script type="text/javascript">
		...
		<%--$(function() {--%>
			...
			<%--$(".info").click(function() {--%>
				...
				<%--$.get("${pageContext.request.contextPath}/adminOrder",{"method":"detail","oid":oid},function(result){--%>
					<%--//result就是响应的json数据--%>
								...
								<c:forEach items="${page.list }" var="o" varStatus="vs">
										<tr>
											...
											<td style="CURSOR: hand; HEIGHT: 22px" align="center"
												...
												<c:if test="${o.state == 1 }">
													<a href="${pageContext.request.contextPath }/adminOrder?method=update&oid=${o.oid}">去发货</a>
												</c:if>
												<c:if test="${o.state == 2 }">
													等待确认收货
												</c:if>
			...
			<div style="text-align: center;">
						<ul class="pagination">
							<c:if test="${page.currentPage != 1 }">
								<li><a href="${pageContext.request.contextPath }/adminOrder?method=page&curPage=${page.currentPage - 1}&state=${param.state}" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a></li>
							</c:if>
							...
							<c:forEach begin="1" end="${page.totalPage }" var="i">
								...
								<c:if test="${page.currentPage != i }">
									<li><a href="${pageContext.request.contextPath }/adminOrder?method=page&curPage=${i}&state=${param.state}">${i }</a></li>
								</c:if>
							</c:forEach>

							<c:if test="${page.currentPage!= page.totalPage }">
								<li>
									<a href="${pageContext.request.contextPath }/adminOrder?method=page&curPage=${page.currentPage+ 1}&state=${param.state}" aria-label="Next">
